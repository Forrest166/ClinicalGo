...
from config import APP_TITLE, checkpoint_path_for_output, default_output_path, ensure_output_prefix, latest_step2a_output
...
class App:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.input_path = tk.StringVar(value=latest_step2a_output())
        ...
        self.progress_text = tk.StringVar(value="Rows: 0/0")
        self.rows_text = tk.StringVar(value="Output rows: 0")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(files, text="Input Excel").grid(row=0, column=0, sticky="w", padx=(0, 8), pady=4)
        ttk.Entry(files, textvariable=self.input_path).grid(row=0, column=1, sticky="ew", pady=4)
        ttk.Button(files, text="Browse", command=self._browse_input).grid(row=0, column=2, padx=(8, 0), pady=4)
        ttk.Label(files, text="Output Excel").grid(row=1, column=0, sticky="w", padx=(0, 8), pady=4)
        ...
        ttk.Label(config, text="Rows / request").grid(row=0, column=3, sticky="w")
        ...
        ttk.Label(runtime, textvariable=self.checkpoint_text).grid(row=1, column=0, columnspan=4, sticky="w", pady=(6, 0))
    ...
    def _dispatch_state(self, state: Dict[str, int]) -> None:
        def writer() -> None:
            processed = int(state["processed_rows"])
            total = max(0, int(state["total_rows"]))
            self.progress_text.set(f"Rows: {processed}/{total}")
            self.rows_text.set(f"Output rows: {state['output_rows']}")
            self.token_text.set(f"Tokens: {state['prompt_tokens']} / {state['completion_tokens']} / {state['total_tokens']}")
            self.checkpoint_text.set(f"Checkpoint: {state['checkpoint_path']}")
    ...
    def _browse_input(self) -> None:
        path = filedialog.askopenfilename(title="Choose input workbook", filetypes=[("Excel workbook", "*.xlsx"), ("All files", "*.*")])
        if path:
            self.input_path.set(path)

    def _browse_output(self) -> None:
        path = filedialog.asksaveasfilename(
            title="Save Step2B output",
            defaultextension=".xlsx",
            filetypes=[("Excel workbook", "*.xlsx")],
            initialfile=Path(self.output_path.get()).name or "Step2B_output.xlsx",
        )
        if path:
            self.output_path.set(ensure_output_prefix(path))
    ...
    def _start_run(self, resume_only: bool) -> None:
        ...
        if not self.input_path.get().strip():
            messagebox.showerror(APP_TITLE, "Please choose an input workbook.")
            return
        try:
            input_rows, sheet_name = load_input_rows(self.input_path.get().strip())
        except Exception as exc:
            messagebox.showerror(APP_TITLE, str(exc))
            return
        self._append_log(f"Selected input sheet `{sheet_name}` with {len(input_rows)} rows.")
        self.output_path.set(ensure_output_prefix(self.output_path.get()))
        ...
        threading.Thread(target=self._run_worker, args=(resume_only, checkpoint_path, input_rows), daemon=True).start()

    def _run_worker(self, resume_only: bool, checkpoint_path: str, input_rows) -> None:
        try:
            if resume_only and not Path(checkpoint_path).exists():
                raise Step2BError("Continue requested, but no checkpoint file exists for this output path.")
            self._append_log(
            ...
            result = process_file(
                ...
                should_stop_fn=lambda: self.stop_requested,
                resume_only=resume_only,
            )
            self.root.after(0, lambda: self.status_text.set("Completed."))
            self._append_log(
                f"Completed. Rows processed: {result['processed_rows']}/{result['total_rows']}. "
                f"Output rows: {result['output_rows']}."
            )
        except UserCancelledError:
            row_count, _ = rebuild_excel_from_checkpoint(checkpoint_path, self.output_path.get().strip(), input_rows)
            ...
            if Path(checkpoint_path).exists():
                ...
                self._append_log(
                    f"Checkpoint status: completed_rows={len(completed_ids)}, rows={row_count}, usage_entries={usage_count}."
                )
